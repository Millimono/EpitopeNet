...
if __name__ == "__main__":
    ...
    print(f"\n✅ NOCROP, (18,18), lr=0.001 → {acc:.4f}")

    result = [{"run": 3, "crop": False, "patches": "[(18,18)]",
               "lr": 0.001, "intensity": False, "acc": acc}]

    drive_path = "/content/drive/MyDrive/ablation_results/"
    os.makedirs(drive_path, exist_ok=True)

    pd.DataFrame(result).to_csv("figs/run3_nocrop_18.csv", index=False)
    with open("figs/run3_nocrop_18.json", "w") as f:
        json.dump(result, f, indent=2)

    save_model(pop, path="figs/model_run3_nocrop_18.pt")

    shutil.copy("figs/run3_nocrop_18.csv",      f"{drive_path}run3_nocrop_18.csv")
    shutil.copy("figs/run3_nocrop_18.json",     f"{drive_path}run3_nocrop_18.json")
    shutil.copy("figs/model_run3_nocrop_18.pt", f"{drive_path}model_run3_nocrop_18.pt")
